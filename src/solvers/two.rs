pub const INPUT: &str = "86	440	233	83	393	420	228	491	159	13	110	135	97	238	92	396
...
pub fn a(input: &str) -> u32 {
    input
        .lines()
        .map(|l| {
            let max = l.split_whitespace()
                .filter_map(|n| n.parse::<u32>().ok())
                .max()
                .unwrap();
            let min = l.split_whitespace()
                .filter_map(|n| n.parse::<u32>().ok())
                .min()
                .unwrap();
            max - min
...
pub fn b(input: &str) -> u32 {
    ...
    input
        ...
        .map(|l| {
            ...
            row.iter()
                .enumerate()
                .flat_map(|(i, &n)| iter::once(n).cycle().zip(row.iter().skip(i + 1)))
                .filter_map(|(a, b)| {
                    if a % b == 0 {
                        Some(a / b)
                    } else if b % a == 0 {
